...
class ShopBankInfoModel {
  ...
  factory ShopBankInfoModel.fromJson(Map<String, dynamic> json) {
    return ShopBankInfoModel(
      id: _toInt(_firstNonNull(json, ['id', 'Id'])),
      shopId: _toInt(_firstNonNull(json, ['shopId', 'ShopId'])),
      bankId: _firstNonNull<String>(json, ['bankId', 'BankId']) ?? '',
      bankName: _firstNonNull<String>(json, ['bankName', 'BankName']) ?? '',
      accountNo: _firstNonNull<String>(json, ['accountNo', 'AccountNo']) ?? '',
      receiverName:
          _firstNonNull<String>(json, ['receiverName', 'ReceiverName']) ?? '',
      isDefault: _firstNonNull<bool>(json, ['isDefault', 'IsDefault']) ?? false,
      isActive: _firstNonNull<bool>(json, ['isActive', 'IsActive']) ?? false,
...
class OrderFullModel {
  ...
  String? statusName;
  String? shopName;
  ShopBankInfoModel? shopBankInfo;
  ...
  factory OrderFullModel.fromJson(Map<String, dynamic> json) {
    final rawDetails =
    ...
    ShopBankInfoModel? parsedShopBankInfo;
    final rawShopBankInfo = _firstNonNull<dynamic>(json, [
      'shopBankInfo',
      'ShopBankInfo',
      'ShopBankAccount',
    ]);
    if (rawShopBankInfo is ShopBankInfoModel) {
      parsedShopBankInfo = rawShopBankInfo;
    } else if (rawShopBankInfo is Map<String, dynamic>) {
      parsedShopBankInfo = ShopBankInfoModel.fromJson(rawShopBankInfo);
    } else if (rawShopBankInfo is Map) {
      parsedShopBankInfo = ShopBankInfoModel.fromJson(
        rawShopBankInfo.cast<String, dynamic>(),
      );
    } else if (rawShopBankInfo is String && rawShopBankInfo.isNotEmpty) {
      try {
        final decoded = jsonDecode(rawShopBankInfo);
        if (decoded is Map) {
          parsedShopBankInfo = ShopBankInfoModel.fromJson(
            decoded.cast<String, dynamic>(),
          );
    ...
    return OrderFullModel(
      ...
      shopBankInfo: parsedShopBankInfo,
    );
  }
  ...
  Map<String, dynamic> toJson() => {
    ...
    'ShopName': shopName,
    'ShopBankInfo': shopBankInfo?.toJson(),
  };

  OrderFullModel copyWith({
    int? id,
    String? code,
    int? customerId,
    String? customerCode,
    ...
    return OrderFullModel(
      ...
      shopBankInfo: shopBankInfo ?? this.shopBankInfo,
    );
  }
}

OrderFullModel parseOrderModel(dynamic data) {
  if (data is OrderFullModel) return data;
  if (data is Map<String, dynamic>) return OrderFullModel.fromJson(data);
  if (data is Map)
    return OrderFullModel.fromJson((data).cast<String, dynamic>());
  if (data is String && data.isNotEmpty) {
    final decoded = jsonDecode(data);
    if (decoded is Map<String, dynamic>)
      return OrderFullModel.fromJson(decoded);
    if (decoded is Map)
      return OrderFullModel.fromJson(decoded.cast<String, dynamic>());
  }
  throw ArgumentError('Invalid order payload');
}

// Giữ nguyên parser cho OrderDetailModel
